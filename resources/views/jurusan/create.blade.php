...
@section('title', 'Tambah Jurusan - SPK Jurusan SMK Babussalam')
    ...
    <div class="d-flex align-items-center">
      ...
      <div>
        <h1 class="h3 mb-0 text-gray-800">
          <i class="ti ti-plus text-primary me-2"></i>Tambah Jurusan Baru
        </h1>
        <p class="text-muted mt-1">Tambahkan data jurusan perguruan tinggi untuk rekomendasi SPK</p>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="card shadow">
      <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">
          <i class="ti ti-building me-2"></i>Form Data Jurusan
        </h6>
      </div>
      <div class="card-body">
        <form action="{{ route('jurusan.store') }}" method="POST">
          ...
          <div class="mb-3">
            ...
            @error('fakultas')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <div class="form-text">Pilih fakultas tempat jurusan berada atau klik + untuk menambah fakultas baru</div>
          </div>
          ...
          <div class="mb-4">
            ...
            @error('perguruan_tinggi')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <div class="form-text">Pilih perguruan tinggi tempat jurusan berada atau klik + untuk menambah perguruan tinggi baru</div>
          </div>
          ...
          <div class="d-flex justify-content-end gap-2">
            ...
            <button type="submit" class="btn btn-primary">
              <i class="ti ti-device-floppy me-1"></i>Simpan Jurusan
            </button>
          </div>
        </form>
      </div>
      ...
      <div class="modal-body">
        <form id="addFakultasForm">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="nama_fakultas" name="nama_fakultas" placeholder="Contoh: Fakultas Teknik Informatika" required>
            <div class="form-text">Masukkan nama fakultas yang belum ada dalam daftar</div>
          </div>
        </form>
      </div>
      ...
      <div class="modal-body">
        <form id="addPerguruanTinggiForm">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="nama_perguruan_tinggi" name="nama_perguruan_tinggi" placeholder="Contoh: Universitas Negeri Jakarta" required>
            <div class="form-text">Masukkan nama perguruan tinggi yang belum ada dalam daftar</div>
          </div>
        </form>
      </div>
      <div class="modal-footer">
        ...
        saveFakultasBtn.addEventListener('click', function() {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    showAlert(data.message || 'Terjadi kesalahan saat menambah fakultas', 'danger');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showAlert('Terjadi kesalahan saat menambah fakultas', 'danger');
            })
            .finally(() => {
                // Reset button state
        ...
        savePerguruanTinggiBtn.addEventListener('click', function() {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    showAlert(data.message || 'Terjadi kesalahan saat menambah perguruan tinggi', 'danger');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showAlert('Terjadi kesalahan saat menambah perguruan tinggi', 'danger');
            })
            .finally(() => {
                // Reset button state
